...
async function seed() {
    ...
    try {
        ...
        const petsData = [
            [tutorIds[0], 'Rex', 'Cão', 'Pastor Alemão', '2020-01-01', 30.5],
            [tutorIds[0], 'Mia', 'Gato', 'Siamês', '2021-06-15', 4.2],
            [tutorIds[1], 'Thor', 'Cão', 'Bulldog', '2019-11-20', 12.0],
            [tutorIds[2], 'Luna', 'Gato', 'Persa', '2022-03-10', 3.8]
        ];

        const petIds = [];
        for (const p of petsData) {
            const res = await client.query(
                'INSERT INTO pets (tutor_id, name, species, breed, birth_date, weight) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id',
                p
            );
        ...
        const productsData = [
            ['Vacina V10', 'Biomédicos', 85.00, 5, 10],
        ...
        await client.query(
            'INSERT INTO appointments (pet_id, vet_id, appointment_date, type, status) VALUES ($1, $2, NOW() - INTERVAL \'2 hours\', \'vacina\', \'concluido\')',
            [petIds[1], vetId]
        );
